...
void CaptureWidget::updateButtons() {
	...
	for (const CaptureButton::ButtonType &t : buttons) {
		...
		if (t == CaptureButton::ButtonType::TYPE_ARROW || t == CaptureButton::ButtonType::TYPE_PENCIL
			|| t == CaptureButton::ButtonType::TYPE_SELECTION || t == CaptureButton::ButtonType::TYPE_CIRCLE)
		{
			connect(b, &CaptureButton::pressed, m_buttonHandler, &ButtonHandler::onShowColorPicker);
		}
...
void CaptureWidget::copyScreenshot() {
	QString fileName = QFileDialog::getSaveFileName(this, QStringLiteral("图片另存为"), "", tr("Images (*.png *.xpm *.jpg *.bmp)"));
